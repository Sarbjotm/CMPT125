...
void swap(int arr[], int i, int j){
	...
	int temp = arr[i];  //Assigning a tmp variable to arr[i]
	arr[i] = arr[j]; //Giving arr[i] val of j
	arr[j] = temp; //Giving arr[j] value of old arr[i]
...
int partition(int arr[], int first, int last){
    int j; //initialize j
    int pivot = arr[last]; //Assign
    int i = (first-1);
    for(j = first; j < last; j++){ //Search the array from first to last
        if(arr[j] <= pivot){ //If j is less than pivot swap
            i = i+1; //Increase i +1
            swap(arr,i,j); //call swap
        }
    }
    swap(arr,i+1,j); //Call swapping function
    return(i+1);
...
void quickSortR(int arr[], int first, int last){
    if(first < last){
        int pi = partition(arr,first,last); //select a partition
...
void merge(int arr[], int first, int mid, int last) {
    ...
    while(left <= mid && right <= last) {
        if (arr[left] == arr[right]){ //if equal assign one of them to -1
            arr[right] = -1;
        }
