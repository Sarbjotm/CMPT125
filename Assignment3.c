...
int findMinpos(int arr[], int mid, int len){
    /*Set up temp index because we keep on comparing mid and mid+1 we won't "Save the value" and will have the last min value instead of the least value which may not be the last min value*/
...
void main(){
    ...
    int mid = length / 2; //calculating mid (random value if you want)
    
    
    //int test2a = findMinpos(arr,mid,length); //Testing code for 2a
    //printf("%d is where min index is of mid-length-1 \n", test2a);
    
    /* Testing code to see before swap
    ...
    /* //Testing code for swap of mid and minindex
     for (int i = mid; i < length; i++){  /
     printf("%d ", arr[i]);
     }
     */
